...
void static LTNCMiner(MinerInfo* miner, const CChainParams& chainparams)
{
    LogPrintf("LightningCash Miner started\n");
    SetThreadPriority (THREAD_PRIORITY_LOWEST);
    RenameThread ("ltnc-miner");
...
void GenerateLTNC(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LTNCMiner, vMiners[i], boost::cref(chainparams)));

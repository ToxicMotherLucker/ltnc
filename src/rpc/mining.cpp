...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
	...
	if (nHeight >= nSpeedFork) {
		...
		while (nMaxTries > 0 && pblock->nNonce < nInnerLoopCount && !CheckProofOfWork(IsYesPower(nHeight) ? pblock->GetHashYespower() : pblock->GetPoWHash(), pblock->nBits, Params().GetConsensus())) {
		    ++pblock->nNonce;
		    --nMaxTries;
		}
	}
	else {
		while (nMaxTries > 0 && pblock->nNonce < nInnerLoopCount && !CheckProofOfWork2(IsYesPower(nHeight) ? pblock->GetHashYespower() : pblock->GetPoWHash(), pblock->nBits, Params().GetConsensus())) {
		    ++pblock->nNonce;
		    --nMaxTries;
		}
	}
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    GenerateLTNC (request.params[0].get_bool(), numCpus, Params());
